"""
Command-Line Interface for FragmentGrow: Fragment-Based Drug Discovery (FBDD) Linking & Growing Agent.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="fragment-based-growing-agent", description="FragmentGrow: Fragment-Based Drug Discovery (FBDD) Linking & Growing Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  FRAGMENTGROW: FRAGMENT-BASED DRUG DISCOVERY (FBDD) LINKING & GROWING AGENT")
        print(f"  Domain: Computational Chemistry & AI Drug Discovery | Standard: Congreve Rule of Three for FBDD")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting FragmentGrow: Fragment-Based Drug Discovery (FBDD) Linking & Growing Agent on http://{args.host}:{args.port}")
